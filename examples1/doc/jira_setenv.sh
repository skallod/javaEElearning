...
JAVA_OPTS="-XX:+UseG1GC -Xms${JVM_MINIMUM_MEMORY} -Xmx${JVM_MAXIMUM_MEMORY} ${JAVA_OPTS} -XX:ReservedCodeCacheSize=1024m  ${JVM_REQUIRED_ARGS} ${DISABLE_NOTIFICATIONS} ${JVM_SUPPORT_RECOMMENDED_ARGS} ${JVM_EXTRA_ARGS} ${JIRA_HOME_MINUSD} ${START_JIRA_JAVA_OPTS}"
JAVA_OPTS="$JAVA_OPTS -Dmail.mime.encodefilename=true -Dmail.mime.encodeparameters=false -Duser.language=ru -Duser.country=RU"
...
#GC_JVM_PARAMETERS="-verbose:gc -Xloggc:./logs/gc_pid_%p.log -XX:+PrintGCDateStamps -XX:+PrintGCDetails -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=10 -XX:GCLogFileSize=100M"
